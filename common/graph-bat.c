...
multiload_graph_bat_inline_output (LoadGraph *g, BatteryData *xd)
{
	...
	if (battery->is_charging) {
		g_snprintf(g->output_str[1], sizeof(g->output_str[1]), "AC");
	} else if (battery->current_now > 1000L*1000L) {
		g_snprintf(
			g->output_str[1],
			sizeof(g->output_str[1]),
			"%.2fA",
...
multiload_graph_bat_tooltip_update (char *buf_title, size_t len_title, char *buf_text, size_t len_text, LoadGraph *g, BatteryData *xd, gint style)
{
	...
	if (style == MULTILOAD_TOOLTIP_STYLE_DETAILED) {
		...
		if (battery->is_critical)
			g_snprintf(buf_text, len_text, "%s %.1f%% (%s)\n%s", _("Current charge:"), battery->percentage, _("Critical level"), status);
		else
			g_snprintf(buf_text, len_text, "%s %.1f%%\n%s", _("Current charge:"), battery->percentage, status);
	} else {
